...
- **Generated vs. faithful neutrals.** Brand colors (primary/secondary/accent)
  are generated into scales from 4 seeds. The neutral surfaces (background,
  card, border, muted, input) are *also* generated by default — but a
  hand-designed theme can't be reproduced that way, so a brand may supply exact
  values via `colors.json → surfaces` (per tone). Overrides win; the engine
  fills any gaps. This is what makes an imported theme (e.g. tweakcn) render true
  instead of looking like the project's own neutrals.
